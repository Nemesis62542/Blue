...
void TerrainSplatBlend(float2 splatBaseUV, FragInputs input, inout TerrainLitSurfaceData surfaceData)
{
    // TODO: triplanar
    // TODO: POM
...
#define SampleResultsTriplanar(i, mask)                                                                         \
    ...
    float mb = min(input.color.x, min(input.color.y, input.color.z));
    ...
    float4 blendMasks0 = input.texCoord2;

    #ifdef _TERRAIN_8_LAYERS
        float4 blendMasks1 = input.texCoord3;
    #else
...
{
    TerrainSplatBlend(uv, input, surfaceData);
}
