...
float3 GetBary (float2 uv) {
    // unity shader code
    float2 texSize = _Control_TexelSize.zw; // size of texture
    float2 stp = _Control_TexelSize.xy;     // size of texel
...
float3 AdjustedNormalTriplanar(float2 uv, float3 positionWS, float3 normal)
{
    float3 dx = ddx(positionWS);
    float3 dy = ddy(positionWS);
    float3 flatNormal = normalize(cross(dy, dx));
    float3 bary = GetBary(uv);
...
half4 SampleTriplanar(half control, float3 positionWS, half3 normal, Texture2D tex, SamplerState samp, half2 tile, half2 offset) {
    if (control < 1e-2f) {
        return 0.0f;
    }
...
void NormalMapMix(float3 positionWS, float3 adjustedNormal, float4 uvSplat01, float4 uvSplat23, inout half4 splatControl, inout half3 mixedNormal)
{
    #if defined(_NORMALMAP)
        float2 _TerrainSizeInv = float2(_TerrainWidthInv, _TerrainHeightInv);
        half3 nrm = 0.0f;
        nrm += splatControl.r * UnpackNormalScale(SampleTriplanar(splatControl.r, positionWS, adjustedNormal, _Normal0, sampler_Normal0, _Splat0_ST.xy * _TerrainSizeInv, _Splat0_ST.zw), _NormalScale0);
        nrm += splatControl.g * UnpackNormalScale(SampleTriplanar(splatControl.g, positionWS, adjustedNormal, _Normal1, sampler_Normal0, _Splat1_ST.xy * _TerrainSizeInv, _Splat1_ST.zw), _NormalScale1);
        nrm += splatControl.b * UnpackNormalScale(SampleTriplanar(splatControl.b, positionWS, adjustedNormal, _Normal2, sampler_Normal0, _Splat2_ST.xy * _TerrainSizeInv, _Splat2_ST.zw), _NormalScale2);
        nrm += splatControl.a * UnpackNormalScale(SampleTriplanar(splatControl.a, positionWS, adjustedNormal, _Normal3, sampler_Normal0, _Splat3_ST.xy * _TerrainSizeInv, _Splat3_ST.zw), _NormalScale3);
...
void SplatmapMix(float3 positionWS, float3 adjustedNormal, float4 uvSplat01, float4 uvSplat23, inout half4 splatControl, out half weight, out half4 mixedDiffuse, out half4 defaultSmoothness, inout half3 mixedNormal)
{
    float2 _TerrainSizeInv = float2(_TerrainWidthInv, _TerrainHeightInv);
    half4 diffAlbedo[4];

    diffAlbedo[0] = SampleTriplanar(splatControl.r, positionWS, adjustedNormal, _Splat0, sampler_Splat0, _Splat0_ST.xy * _TerrainSizeInv, _Splat0_ST.zw);
    diffAlbedo[1] = SampleTriplanar(splatControl.g, positionWS, adjustedNormal, _Splat1, sampler_Splat0, _Splat1_ST.xy * _TerrainSizeInv, _Splat1_ST.zw);
    diffAlbedo[2] = SampleTriplanar(splatControl.b, positionWS, adjustedNormal, _Splat2, sampler_Splat0, _Splat2_ST.xy * _TerrainSizeInv, _Splat2_ST.zw);
    diffAlbedo[3] = SampleTriplanar(splatControl.a, positionWS, adjustedNormal, _Splat3, sampler_Splat0, _Splat3_ST.xy * _TerrainSizeInv, _Splat3_ST.zw);
    ...
    NormalMapMix(positionWS, adjustedNormal, uvSplat01, uvSplat23, splatControl, mixedNormal);
}
...
void ComputeMasks(half4 splatControl, float3 adjustedNormal, out half4 masks[4], half4 hasMask, Varyings IN)
{
    masks[0] = 0.5h;
    masks[1] = 0.5h;
    masks[2] = 0.5h;
...
#ifdef _MASKMAP
    float2 _TerrainSizeInv = float2(_TerrainWidthInv, _TerrainHeightInv);
    masks[0] = lerp(masks[0], SampleTriplanar(splatControl.r, IN.positionWS, adjustedNormal, _Mask0, sampler_Mask0, _Splat0_ST.xy * _TerrainSizeInv, _Splat0_ST.zw), hasMask.x);
    masks[1] = lerp(masks[1], SampleTriplanar(splatControl.g, IN.positionWS, adjustedNormal, _Mask1, sampler_Mask0, _Splat1_ST.xy * _TerrainSizeInv, _Splat1_ST.zw), hasMask.y);
    masks[2] = lerp(masks[2], SampleTriplanar(splatControl.b, IN.positionWS, adjustedNormal, _Mask2, sampler_Mask0, _Splat2_ST.xy * _TerrainSizeInv, _Splat2_ST.zw), hasMask.z);
    masks[3] = lerp(masks[3], SampleTriplanar(splatControl.a, IN.positionWS, adjustedNormal, _Mask3, sampler_Mask0, _Splat3_ST.xy * _TerrainSizeInv, _Splat3_ST.zw), hasMask.w);
#endif
...
#endif
{
    ...
    float3 adjustedNormal = AdjustedNormalTriplanar(IN.uvMainAndLM.xy, IN.positionWS, IN.normal.xyz);

    half4 hasMask = half4(_LayerHasMask0, _LayerHasMask1, _LayerHasMask2, _LayerHasMask3);
    half4 masks[4];
    ComputeMasks(splatControl, adjustedNormal, masks, hasMask, IN);
    ...
    SplatmapMix(IN.positionWS, adjustedNormal, IN.uvSplat01, IN.uvSplat23, splatControl, weight, mixedDiffuse, defaultSmoothness, normalTS);
